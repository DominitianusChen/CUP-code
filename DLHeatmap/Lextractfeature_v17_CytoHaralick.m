...
for j = 1:length(idxUse)
 ...
 try
        ...
        if exist(strAllFeatsMat,'file')~=2
            ...
            str=sprintf('%s%sFullFeatures_IM%s.mat',strSavePath,[curPID '/'],curName);
...
%             end
            ...
            if exist(str,'file')~=2
             ...
             %% extract Cytoplasm Haralick  features
                ...
                if exist(str,'file')~=2
                    disp('begin to exract Cytoplasm Haralick feature in nuclei;');
                    [feats,description] = Lharalick_img_nuclei_wise(rgb2gray(I),bwCyto,Num_level4Haralick_nuclei_wise);% Check results' shape
                    feats = feats';
                    description = strcat('Cyto',description');
                    save(str,'feats','description','-v7.3');
                    disp('Cytoplasm haralick nuclei wise result in nuclei saved');
                else
                    disp('Cytoplasm haralick nuclei wise result in nuclei saved found');
                    %                 load(str);
                end
            %% extract Fractal features
