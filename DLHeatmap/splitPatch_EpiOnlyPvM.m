...
[uniquePatient,ia,ic] = unique(topID);
trainSet = randperm(113,75)';
tS = ones(length(uniquePatient),1);
tS(trainSet) = 0;
%% main loop
testStatus = tS(ic);
...
saveMainPath = strcat('E:\MvP\FinalExperimentWithPancreas\NewRNG\Run3\DeepLearningHeatmap\PvM\');
...
for j = 1:length(organs)
    ...
    for i = 1:length(fileDir)
        ...
        %             continue
        %         end
        testStatus = strcmp(fullStatus(fullStatusIdx,3),"1");% 0:train
        ...
        if testStatus
            sfp = strcat(saveMainPath,'Test\');
        else
            sfp = strcat(saveMainPath,'Train\');
        end
        organStatus = char(metStatus(fullStatusIdx));
        priStatus = strcmp(organStatus(end),'P');% 0:Mets
        if priStatus
            sp = strcat(sfp,'Pri\');
            thres = 125;
        else
