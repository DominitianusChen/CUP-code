...
class_names=["LGD", "HGD"]#what classes we expect to have in the data, here we have only 2 classes but we could add additional classes
...
# +
files=glob.glob('./**/*.png') # create a list of the files, in this case we're only interested in files which have masks so we can use supervised learning
...
valTable = tables.open_file('BarretsEsoDysplasia_val.pytable')
trainTable = tables.open_file('BarretsEsoDysplasia_train.pytable')
