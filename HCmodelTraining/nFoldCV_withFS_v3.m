...
%             catch
%                 [IDX, ~] = rankfeatures(training_set',training_labels,'criterion',params.fsname);
%             end
            [IDX, ~] = rankfeatures(training_set',training_labels,'criterion',params.fsname,'CCWeighting', 1); %%CCWeighting is the parameter that weights by correlation w/ previously selected features
            if length(find(IDX==1))>1 && size(data_set,2)>20 %occurs if CCWeighting is too high and not many features to choose from or if too few (<20) features in data_set
                ...
                if ~isempty(1:length(IDX)-length(find(IDX==1)))
        ...
        %[temp_stats,~] = Classify_wrapper(params.classifier, training_set(:,fea) , testing_set(:,fea), training_labels(:), testing_labels(:), params.classifieroptions);
        if strcmp(params.classifier,'SVM')
            [temp_stats,~] = Classify(params.classifier, training_set(:,fea) , testing_set(:,fea), training_labels(:), testing_labels(:), params.classifieroptions);
        else
    ...
    if numel(unique(testing_labels)) > 1 %won't work for Leave One Out CV
        ...
        stats(j).tp = ConfTemp(:,1);
        stats(j).tn = ConfTemp(:,2);
        stats(j).fp  = ConfTemp(:,3);
        stats(j).fn = ConfTemp(:,4);
        stats(j).acc = (stats(j).tp+stats(j).tn )./sum(ConfTemp,2);
        stats(j).ppv = stats(j).tp./(stats(j).tp+stats(j).fp);
        stats(j).sens = stats(j).tp./(stats(j).tp+ConfTemp(:,4));
        stats(j).spec = ConfTemp(:,2)./(stats(j).fp+ConfTemp(:,2));
        stats(j).X = X;
        stats(j).Y = Y;
        stats(j).T = T;
        Pre = ((stats(j).tp+stats(j).fp).*(stats(j).tp+ConfTemp(:,4)) + (ConfTemp(:,2)+ConfTemp(:,4)).*(ConfTemp(:,2)+stats(j).fp)) ./ (stats(j).tp+ConfTemp(:,2)+stats(j).fp+ConfTemp(:,4)).^2;
        stats(j).kappa = (stats(j).acc - Pre) ./ (1 - Pre);
        stats(j).Fscore = 2.*stats(j).tp./(2.*stats(j).tp+stats(j).fp+ConfTemp(:,4));
