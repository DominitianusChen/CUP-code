function features = loadExtractedResult0912UMAP(imgLoc,featureLoc)
%global imgLoc
...
if size(caseID,2) > size(caseID,1)
    caseID = caseID';
end
caseID = caseID(:,1);
...
%
split = strsplit(featureLoc,'\');
featFamily = split{end};
featurePath = strcat(featureLoc,['\umap_Embedded_',featFamily,'.mat']);
featuresMat = load(featurePath);
featuresMat = featuresMat.UMAP;
