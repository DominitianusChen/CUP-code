...
function Lextractfeature_v17_MvP_256(idxBegin,idxEnd,strWSIPath,flag_epistroma,strPathIM,strSavePath,strEpiStrMaskPath,strNucleiMaskPath,flag_have_nulei_mask,flag_nuclei_cluster_finding,image_format,flag_Hosoya,flag_haralick,flag_CCM,flag_WSI,para_nulei_scale_low,para_nulei_scale_high,flagRandomPatch)
...
if ~isempty(idxUse)
    ...
    for j = 1:length(idxUse)
        ...
        try
            ...
            strAllFeatsMat=sprintf('%s%s%s_allFeats.mat',strSavePath,[curPID '/'],curName);
            ...
            if exist(strAllFeatsMat,'file')~=2
                ...
                if exist(strSaveCur,'file')~=2
                    if flag_have_nulei_mask
                        bwNuclei=logical(imread([strNucleiMaskPath curName]));%show(bwNuclei)%LshowBWonIM(bwNuclei,I);
                        ...
                        [I_norm, ~, ~] = normalizeStaining(I);%show(I) show(I_norm);
                        I_norm=I_norm(:,:,1);
                ...
                if flag_epistroma
                    %             strSaveCur=sprintf('%s%s%s_nuclei.mat',strSavePath,[curPID '/'],curName);
                    strSave=sprintf('%s%snuclei_label_in_epistroma_IM%s.mat',strSavePath,[curPID '/'],curName);
                    ...
                    if exist(strSave,'file')~=2
                        if flag_have_nulei_mask
                            ...
                            bwNuclei=logical(imread([strNucleiMaskPath curName]));%show(bwNuclei)
                    ...
                    str=sprintf('%s%sHaralickNucleiWiseFeatures_IM%s_epistroma.mat',strSavePath,[curPID '/'],curName);
                    ...
                    str=sprintf('%s%sFractalFeatures_IM%s_epistroma.mat',strSavePath,[curPID '/'],curName);
                    ...
                    str=sprintf('%s%sCRLFeatures_IM%s_epistroma.mat',strSavePath,[curPID '/'],curName);
                    ...
                    str=sprintf('%s%sBasicShapeFeatures_IM%s_epistroma.mat',strSavePath,[curPID '/'],curName);
                    ...
                    str=sprintf('%s%sBasicShapeinCCGFeatures_IM%s_epistroma.mat',strSavePath,[curPID '/'],curName);
                    if exist(str,'file')~=2
                        %%
                    ...
                    str=sprintf('%s%sFullFeatures_IM%s_epistroma.mat',strSavePath,[curPID '/'],curName);
                    ...
                    if flag_CCM
                        ...
                        str=sprintf('%s%sCCMFeatures_IM%s_epistroma.mat',strSavePath,[curPID '/'],curName);
                        ...
                        %% extract the cCCM
                        str=sprintf('%s%scCCMFeatures_IM%s_epistroma.mat',strSavePath,[curPID '/'],curName);
                        para.CGalpha_min=0.38; para.CGalpha_max=0.5;
                        para.alpha_res=0.02;% larger the alpha, sparse the graph
                        para.radius=0.2;
                        if exist(str,'file')~=2
                ...
                %% extract all features for non-epi
...
%                 end
                ...
                if exist(str,'file')~=2
                    ...
                    feats = feats';
                    description = strcat('CytoHara-',description');
                    save(str,'feats','description','-v7.3');
...
%                 end
                ...
                cur_tempmat=sprintf('%s%s*%s*.mat',strSavePath,[curPID '/'],curName(1:end-length(image_format)));
                            ...
                            if exist(strAllFeatsMat_name,'file')~=2
                                ...
                                for i_des=1:length(description_stroma)
                                    ...
                                    if iscell(description_epi)
                                        ...
                                        feature_list_epi{idx}=['ST:' description_stroma{i_des}];
                                    end
                ...
                %% Avoid saving feature descriptions
%                 if exist(strAllFeatsMat_name,'file')~=2
%                     save(strAllFeatsMat_name,'alldescription','-v7.3');
%                 end
                %         clear allFeats
                %         clear alldescription
                %delete([strPathIM curIM '.png'])
            else
                if exist([strPathIM curIM '.png'],'file')
                    %delete([strPathIM curIM '.png'])
                end
            end
            ...
            %         fclose(fid);
            fprintf('The mask idx is %d, job Error. Image name is %s,\n\n',i,curName)
        end
    end
else
    try
        fid = fopen(fullfile(pwd, strcat('featExtraction-Finished-',date,'.txt'))...
            ,'a');
        fprintf(fid,'All patches extracted for Image: %s,\n',extractBefore(curName,'_'));
        fclose(fid);
    catch
    end
end
